...
struct CodexExecutableLocator: Sendable {
  ...
  init(
    ...
    var resolved: [URL] = []
    if let explicit = environment["BORING_NOTCH_CODEX_PATH"], !explicit.isEmpty {
      resolved.append(URL(fileURLWithPath: explicit))
    } else if let legacy = environment["COWLICK_CODEX_PATH"], !legacy.isEmpty {
      resolved.append(URL(fileURLWithPath: legacy))
    }
